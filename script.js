...
(function () {
  ...
  /* ---------- 2c) YETENEKLER — DAKTİLO (TYPEWRITER) METNİ ----------
     ...
     titles/TYPE_SPEED gibi değişkenleriyle hiç kesişmiyor. */
  (function skillsTypewriterModule() {
    var textEl = document.getElementById("skillsTypewriterText");
    if (!textEl) return;
    ...
    // Hareketi azalt tercihinde: animasyon yok, ilk cümle sabit görünür
    if (reduceMotion) {
      textEl.textContent = sentences[0];
      return;
    }

    var TYPE_SPEED = 32;    // harf yazma hızı (ms)
    var ERASE_SPEED = 18;   // harf silme hızı (ms)
    var HOLD_FULL = 2200;   // tamamlanan cümlenin ekranda kalma süresi
    var HOLD_EMPTY = 400;   // sonraki cümleye geçmeden önceki bekleme
    ...
    function tick() {
      ...
      if (!erasing) {
        ...
        if (charIndex === 0) {
          erasing = false;
          sentenceIndex = (sentenceIndex + 1) % sentences.length; // sonsuz döngü
          delay = HOLD_EMPTY;
        } else {
          delay = ERASE_SPEED;
        }
